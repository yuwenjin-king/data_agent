...
def test_build_multi_turn_context_includes_recent_messages(db_session):
    session = chat_session_crud.create(
        db_session, obj_in=ChatSessionCreate(agent_id=1, title="test")
    )

    for i in range(15):
        chat_message_crud.create(
            db_session,
            obj_in=ChatMessageCreate(session_id=session.id, role="user", content=f"question {i}"),
        )
        chat_message_crud.create(
            db_session,
            obj_in=ChatMessageCreate(
                session_id=session.id, role="assistant", content=f"answer {i}"
            ),
        )
