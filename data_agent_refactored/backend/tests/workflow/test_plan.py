...
async def test_plan_executor_routes_to_first_step():
    plan = _plan_json([
        ...
        {"step": 2, "tool_to_use": "report_generator",
...
async def test_plan_executor_exhausted_routes_to_report():
    plan = _plan_json([{"step": 1, "tool_to_use": "sql_generate",
...
async def test_plan_executor_rejects_unsupported_tool():
    plan = _plan_json([{"step": 1, "tool_to_use": "drop_table",
                        "tool_parameters": {"instruction": "x"}}])
...
async def test_semantic_consistency_no_llm_passes():
    out = await semantic_consistency_node(
        {"sql_generate_output": "SELECT 1"}, {"configurable": {}})
