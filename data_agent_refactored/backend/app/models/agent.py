...
class Agent(Base):
    ...
    status = Column(
        String(50), default="draft", comment="状态：draft-待发布，published-已发布，offline-已下线"
    )
    ...
    business_knowledge = relationship(
        "BusinessKnowledge", back_populates="agent", cascade="all, delete-orphan"
    )
    semantic_models = relationship(
        "SemanticModel", back_populates="agent", cascade="all, delete-orphan"
    )
    agent_knowledge = relationship(
        "AgentKnowledge", back_populates="agent", cascade="all, delete-orphan"
    )
    agent_datasources = relationship(
        "AgentDatasource", back_populates="agent", cascade="all, delete-orphan"
    )
    preset_questions = relationship(
        "AgentPresetQuestion", back_populates="agent", cascade="all, delete-orphan"
    )
