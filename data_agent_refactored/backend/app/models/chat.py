...
class ChatSession(Base):
    ...
    status = Column(
        String(50), default="active", comment="状态：active-活跃，archived-归档，deleted-已删除"
    )
...
class ChatMessage(Base):
    ...
    role = Column(
        String(20), nullable=False, comment="角色：user-用户，assistant-助手，system-系统"
    )
    content = Column(Text, nullable=False, comment="消息内容")
    message_type = Column(
        String(50),
        default="text",
        comment="消息类型：text-文本，sql-SQL查询，result-查询结果，error-错误",
    )
