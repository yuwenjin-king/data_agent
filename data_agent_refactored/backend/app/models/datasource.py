...
class Datasource(Base):
    ...
    agent_datasources = relationship(
        "AgentDatasource", back_populates="datasource", cascade="all, delete-orphan"
    )
    logical_relations = relationship(
        "LogicalRelation", back_populates="datasource", cascade="all, delete-orphan"
    )
...
class AgentDatasource(Base):
    ...
    datasource_id = Column(
        Integer, ForeignKey("datasource.id", ondelete="CASCADE"), nullable=False, comment="数据源ID"
    )
    ...
    tables = relationship(
        "AgentDatasourceTables", back_populates="agent_datasource", cascade="all, delete-orphan"
    )
...
class AgentDatasourceTables(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    agent_datasource_id = Column(
        Integer,
        ForeignKey("agent_datasource.id", ondelete="CASCADE"),
        nullable=False,
        comment="智能体数据源ID",
    )
