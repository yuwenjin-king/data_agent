...
class CRUDAgent(CRUDBase[Agent, AgentCreate, AgentUpdate]):
    ...
    def search(self, db: Session, *, keyword: str, skip: int = 0, limit: int = 100) -> List[Agent]:
        return (
            db.query(Agent)
            .filter((Agent.name.contains(keyword)) | (Agent.description.contains(keyword)))
            .offset(skip)
            .limit(limit)
            .all()
        )
...
class CRUDBusinessKnowledge(
    CRUDBase[BusinessKnowledge, BusinessKnowledgeCreate, BusinessKnowledgeUpdate]
):
    def get_multi_by_agent(
        ...
        return (
            ...
            .filter(and_(BusinessKnowledge.agent_id == agent_id, BusinessKnowledge.is_deleted == 0))
            .offset(skip)
            .limit(limit)
            .all()
        )
